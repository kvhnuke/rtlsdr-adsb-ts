...
extern void callback(char updown, uint8_t *data, int len,
                     int rs_errors, int signal_strength);
...
static void dump_raw_message(char updown, uint8_t *data, int len,
                             int rs_errors)
{
  callback(updown, data, len, rs_errors, signal_strength);
}

static void handle_adsb_frame(uint64_t timestamp, uint8_t *frame, int rs)
{
  dump_raw_message('-', frame, (frame[0] >> 3) == 0 ? SHORT_FRAME_DATA_BYTES : LONG_FRAME_DATA_BYTES,
                   rs);
  fflush(stdout);
}
...
{ // sets signal_strength to scaled amplitude. 0 = no signal, 1000 = saturated receiver on all samples in measurement.
  long avg = 0;
  int n = len;
  while (n--)
  {
...
static char buffer[65536 * 2];
static uint16_t phi[65536];

int EMSCRIPTEN_KEEPALIVE demodulate(char *data, int dlen)
{
  int n;
  int processed;
  int doffset = 0;
